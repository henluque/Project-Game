...
function Helga.update(dt, player)
  ...
  -- -------------------------------------------------------
  -- 1. SE HELGA MORREU: só avança a animação de death e sai
  -- -------------------------------------------------------
  if not Helga.vivo then
    ...
    if Helga.tempo_animacao >= Helga.frame_delay then
      ...
      if Helga.frame_atual > #anim.quads then
        Helga.frame_atual = #anim.quads  -- trava no último frame
      end
    end
    return
  end
  ...
  -- -------------------------------------------------------
  -- 2. SE PLAYER MORREU: fica em idle e sai
  -- -------------------------------------------------------
  ...
  if hitbox and colide(helga_corpo, hitbox) and Helga.pode_tomar_dano then
    ...
    if Helga.vida <= 0 then
      Helga.estado = "death"
      Helga.vivo = false
      Helga.setAnim("death")  -- força a troca imediata
    else
      Helga.estado = "hit"
      Helga.setAnim("hit")    -- força a troca imediata
      Helga.x = Helga.x + (player.direcao * 60)
    end
  end

  -- -------------------------------------------------------
  -- 5. LÓGICA DE IA (bloqueada durante hit/death)
  -- -------------------------------------------------------
  if Helga.estado ~= "hit" and Helga.estado ~= "death" and Helga.estado ~= "attack" then
    local dx = player.x - Helga.x
    local dy = player.y - Helga.y
    ...
    elseif dist_y < 40 and Helga.pode_atacar then
      Helga.estado = "attack"
      Helga.animacao_atual = ""  -- <-- força setAnim a resetar sempre
      Helga.atirou = false       -- <-- garante que vai atirar neste ciclo

    elseif dist_y < 40 and not Helga.pode_atacar then
      Helga.estado = "idle"
  ...
  -- -------------------------------------------------------
  -- 6. SINCRONIZA ANIMAÇÃO COM ESTADO (ANTES de avançar frame)
  -- -------------------------------------------------------
  ...
  if Helga.tempo_animacao >= Helga.frame_delay then
    ...
    if Helga.frame_atual > limite then
      if Helga.estado == "hit" then
        Helga.estado = "idle"
        Helga.setAnim("idle")
      elseif Helga.estado == "attack" then
        Helga.estado = "idle"
        Helga.setAnim("idle")
        -- NÃO mexa no atirou aqui
      elseif Helga.estado == "death" then
        Helga.frame_atual = limite
      else
        Helga.frame_atual = 1
      end
    end
  ...
  -- -------------------------------------------------------
  -- 9. PROJÉTEIS: movimento, animação e colisão
  -- -------------------------------------------------------
...
function Helga.draw()
  -- Desenha enquanto viva OU durante a animação de death
  ...
  -- debug: hitbox (remova quando não precisar mais)
